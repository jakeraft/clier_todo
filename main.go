...
func main() {
	...
	case "list":
		doneOnly := len(os.Args) > 2 && os.Args[2] == "--done"
...
func printUsage() {
	...
	fmt.Println("  todo list [--done]  List todos (--done shows completed)")
...
func listTodos(db *sql.DB, doneOnly bool) error {
	var query string
	if doneOnly {
		query = "SELECT id, title, done FROM todos WHERE done = 1"
	} else {
		query = "SELECT id, title, done FROM todos WHERE done = 0"
	}
